...
export function matchStudentField(s, q) {
  ...
  // combined name fields (ชื่อ+นามสกุล, prefix+ชื่อ+นามสกุล)
  const fullNameTh   = str(`${s.name} ${s.lastname}`);
  const fullNameThPx = str(`${s.prefix}${s.name} ${s.lastname}`);
  const fullNameEn   = str(`${s.nameEn} ${s.lastnameEn}`);
