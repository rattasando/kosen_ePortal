...
// แปลงค่าบางฟิลด์ให้อ่านเข้าใจง่าย ก่อนเอาไปแสดงใน history
const FIELD_VALUE_FORMATTERS = {
  selfFunded: (v) => (v === true || v === "true") ? "จ่ายเอง" : "รับทุน",
};

function formatFieldValue(key, raw) {
  const fmt = FIELD_VALUE_FORMATTERS[key];
  return fmt ? fmt(raw) : (str(raw) || "—");
}
